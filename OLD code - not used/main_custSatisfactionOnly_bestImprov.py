...
def two_opt_improvement(
    ...
    while improvement_found and iteration < max_iterations:
        ...
        best_iteration_route = best_route.copy()
        best_iteration_arrival_times = arrival_times.copy()
        ...
        for i in range(1, len(best_route) - 2):
            for j in range(i + 1, len(best_route) - 1):
                ...
                new_total_distance = sum(
                    distance(new_route[k], new_route[k + 1]) for k in range(len(new_route) - 1)
                )

                new_satisfaction_cost = sum(
                    (1.0 - calculate_satisfaction(
                        new_arrival_times[k],
                        new_route[k].time_windows[day_index],
                        buffer_minutes
                ...
                # Track the best improvement within this iteration
